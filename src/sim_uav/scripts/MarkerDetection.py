...
class MarkerDetector:
    ...
    def Aruco_marker_detector(self):
        ...
        if markerCorners:
            ...
            ret1 = cv.aruco.estimatePoseSingleMarkers( markerCorners, markerSize, self.K, self.distCoeffs)
            rvecs, tvecs = ret1[0][0,0,:],ret1[1][0,0,:]
            # Draw the axis on the marker
	    #frame_out = cv.aruco.DetectedMarker(img, markerCorners, markerIds)
            ...
            #tvecs = np.matmul(rotMat, tvecs.T)
	    ...
	    #test
	    marker_pos = PT()
            marker_pos.position.x = -tvecs[1]
            marker_pos.position.y = -tvecs[0]
            marker_pos.position.z = tvecs[2]
