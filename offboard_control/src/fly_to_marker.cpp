...
{   
    ...
    while(ros::ok() && !stop)
    {
        ...
        {   
            ...
            pid_cmd.setUMax(0.2);
            pid_cmd.setUMin(-0.2);
            pid_cmd1.setUMax(0.2);
            pid_cmd1.setUMin(-0.2);
            pid_cmd2.setUMax(0.05);
            pid_cmd2.setUMin(-0.05);
            geometry_msgs::PoseStamped target_point;
            target_point.pose.position.x = 0;
            target_point.pose.position.y = 0;
            // target_point.pose.position.z = 2.0;
            bool check_target_marker = false;
            while (!check_target_marker && ros::ok())
            {   
                ...
                cmd_vel.publish(a);
                check_target_marker = check_marker(error, marker_position, target_point);
                // cout << check_target << endl;
                ros::spinOnce();
                rate.sleep();
            }
            stop = true;
